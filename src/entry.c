                         ...
                         uint64_t key_size, uint64_t value_size)
{
    ...
    encode_u64_le(out + ENTRY_HEADER_KEY_SIZE_OFFSET, key_size);
    encode_u64_le(out + ENTRY_HEADER_VALUE_SIZE_OFFSET, value_size);
...
bool entry_header_decode(entry_header_t *out, const uint8_t in[ENTRY_HEADER_SIZE])
{
    ...
    out->key_size = decode_u64_le(in + ENTRY_HEADER_KEY_SIZE_OFFSET);
    out->value_size = decode_u64_le(in + ENTRY_HEADER_VALUE_SIZE_OFFSET);
